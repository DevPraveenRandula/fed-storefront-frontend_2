...
import { Link, useSearchParams } from "react-router"; // FIX: Correct import
...
function CompletePage() {
  ...
  // FIX: Handle error or missing data
  if (error || !data) {
    return <main className="px-8"><p className="text-red-500">Error: Order not found</p></main>;
  }
  ...
  return (
    <main className="px-8">
      ...
      <div className="mt-4">
        <p>
          Total Price: $
          {data.items.reduce((acc, item) => acc + item.product.price * item.quantity, 0)}
        </p>
      </div>
